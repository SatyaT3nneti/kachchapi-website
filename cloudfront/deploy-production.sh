...
# Deploy Kachchapi website to AWS CloudFront (Production)
# Domain: kachchapi.com

set -e

BUCKET_NAME="kachchapi-production-website"
DISTRIBUTION_DOMAIN="kachchapi.com"
REGION="ap-south-2"
BUILD_DIR="build"

echo "🚀 Deploying Kachchapi website to AWS CloudFront (PRODUCTION)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
if ! aws s3 ls "s3://$BUCKET_NAME" 2>&1 | grep -q 'NoSuchBucket'; then
  ...
  cat > /tmp/bucket-policy-prod.json <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
EOF
  ...
  aws s3api put-bucket-policy --bucket $BUCKET_NAME --policy file:///tmp/bucket-policy-prod.json
...
if [ -z "$DIST_ID" ] || [ "$DIST_ID" == "None" ]; then
  ...
  echo "   - SSL Certificate: Request in ACM (us-east-1 for CloudFront) for kachchapi.com"
  echo "   - Default root object: index.html"
  echo "   - Error pages: 404 -> /index.html (200)"
  echo ""
  echo "   To create using CloudFormation:"
  echo "   aws cloudformation create-stack --stack-name kachchapi-production --template-body file://cloudfront/cloudformation-production.yaml --parameters ParameterKey=CertificateArn,ParameterValue=YOUR_CERT_ARN"
else
...
echo "   - Domain: $DISTRIBUTION_DOMAIN"
echo "   - SSL Certificate from ACM (us-east-1) for kachchapi.com"
echo "   - Origin: s3://$BUCKET_NAME"
echo "2. Update DNS to point $DISTRIBUTION_DOMAIN to CloudFront"
